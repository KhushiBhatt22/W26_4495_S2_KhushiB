...
import { Sparkles, Type, Eye, Maximize2, Save, PenLine, GitMerge, CheckCircle } from "lucide-react";
...
const ChapterEditorTab = ({
  ...
  const [writingMode, setWritingMode] = useState("manual");
  const [saveSuccess, setSaveSuccess] = useState(false);

  const handleManualSave = () => {
    onSaveChapter(selectedChapterIndex);
    setSaveSuccess(true);
    setTimeout(() => setSaveSuccess(false), 2500);
  };

  const handleCompleteWithAI = () => {
    onCompleteChapterContent(selectedChapterIndex, currentChapter.content);
  };

  const isCurrentlyGenerating = isGenerating === selectedChapterIndex;
  const isCurrentlyCompleting = isCompleting === selectedChapterIndex;
      ...
      <div className="h-full bg-white px-8 py-6">
        <div className="h-full bg-white">
          <div className="space-y-6 h-full flex flex-col">
              ...
              <div className="flex items-center gap-3">
                {writingMode === "manual" && (
                  <button onClick={handleManualSave}
                    className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${saveSuccess ? "bg-green-50 text-green-700 border border-green-300" : "bg-blue-600 hover:bg-blue-700 text-white"
                      }`}>
                    {saveSuccess ? <><CheckCircle className="w-4 h-4" /> Saved!</> : <><Save className="w-4 h-4" /> Save Chapter</>}
                  </button>
                )}

                {writingMode === "ai-full" && (
                  <Button onClick={() => onGenerateChapterContent(selectedChapterIndex)} isLoading={isCurrentlyGenerating} icon={Sparkles} size="sm">
                ...
                {writingMode === "ai-complete" && (
                  <div className="flex items-center gap-2">
                    <button onClick={handleManualSave}
                      className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border transition-all ${saveSuccess ? "bg-green-50 text-green-700 border-green-300" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
                        }`}>
                      {saveSuccess ? <><CheckCircle className="w-4 h-4" /> Saved!</> : <><Save className="w-4 h-4" /> Save Draft</>}
                    </button>
                    <Button onClick={handleCompleteWithAI} isLoading={isCurrentlyCompleting} icon={GitMerge} size="sm"
                      disabled={!currentChapter.content || currentChapter.content.trim().length < 10}>
                      Complete with AI
                    </Button>
                  </div>
                )}
            ...
            {/* Tip for half-half mode */}
            {writingMode === "ai-complete" && (
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-4 py-3 text-sm text-emerald-700">
                <strong>How it works:</strong> Write your opening idea above, then click <strong>"Complete with AI"</strong> — AI will continue in your style.
              </div>
            )}
